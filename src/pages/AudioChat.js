...
import { userProfile, audioCall, audioCallUser, videoCall } from "../features/userSlice";
import { changeImageLinkDomain, checkLiveDomain, removeVideoListRightClick, restrictBack, returnDefaultImage } from "../commonFunctions";
import NotificationManager from "react-notifications/lib/NotificationManager";
import { STREAMER_REPORT_API } from "../components/Api";
...
let videoCallStatus = 0, videoCallParams, interval,
  ...
  manageCoinsTimeViewsInterval, manageCoinsTimeViewsCounter = 0, goHost = false, goAud = false, hostCallCheck = true;
...
const AudioChat = () => {
  ...
  useEffect(() => {
    ...
    if (!params.channel_name) {
      ...
      if (!localStorage.getItem("videoCallPageRefresh")) {
        ...
        console.log(params.receiver == "false" ? videoCallState : null, "----------videoCallState---------")
        ...
        SOCKET.off('get_connected_people_video_call').on('get_connected_people_video_call', (data) => {
          if ((data.user_from_id == videoCallParams.user_from_id && data.user_to_id == videoCallParams.user_to_id)
            ...
            if (!!changeConnectedPeople && !!latestConnectedPeople) {
              console.log(changeConnectedPeople.time_from_id, latestConnectedPeople.time_from_id, "----", changeConnectedPeople.time_to_id, latestConnectedPeople.time_to_id, "oooooooooooooooooooooooooooo")
        ...
        SOCKET.off('sender_show_video_call').on('sender_show_video_call', (data) => {
          if ((data.user_from_id == videoCallParams.user_from_id && data.user_to_id == videoCallParams.user_to_id)
            ...
            if (!!userData && (data.user_from_id == userData.user_id)) {
              ...
              const volume = false;
              joinChannelAudio('audience', option, volume, "i_am_aud")
        ...
        SOCKET.off('authorize_video_call').on('authorize_video_call', (data) => {
          if ((data.user_from_id == videoCallParams.user_from_id && data.user_to_id == videoCallParams.user_to_id)
            ...
            if (!hostCallCheck) {
              ...
              if (!!userData && (data.user_from_id == userData.user_id)) {
                if (data.show_volla_alert) {
                  ...
                  toastFlash("Volla !! Earned 50 diamonds right away. You will earn 50 diamonds on each minute.", toastFlashOptions)
                }
              }
            ...
            if (!!userData && (data.user_to_id == userData.user_id)) {  // receiver
              timerVar = setInterval(countTimer, 1000);
              ...
              const volume = true;
              joinChannelAudio('audience', option, volume, "i_am_aud");
              joinChannelAudio('host', option, volume);
              interval = window.setInterval(() => {
                var list = document.getElementById("remote_video_");
                if (!!list) {
                  if (list.childNodes.length > 1) {
            ...
            if (!!userData && (data.user_from_id == userData.user_id)) { // host..
              if (hostCallCheck) {
                ...
                audioManageCoinsTimeViews(show_host_coins)
  ...
  const audioManageCoinsTimeViews = (show_host_coins) => {
  ...
  return (
    <section className="home-wrapper">
              ...
              <div className="tab-top d-flex flex-wrap-wrap align-items-center">
                <div className="vc-action-tab ml-auto mr-4 position-relative">
                  ...
                  <ul className="action-menu" id="actionBtn">
                    ...
                    <a href="javascript:void(0)" onClick={() => endCall(true)}>End Audio</a>
                
                </li> */}
                  </ul>
                  <a href="javascript:void(0)" className="end-calls btn bg-grd-clr" onClick={() => endCall(true)}>End Audio</a>
                </div>
                <NavLinks />
              </div>
            </div>
      ...
      <div className="vc-screen-wrapper">
        ...
        {/* {
            ...
            <div className="charges-reminder-txt">
              <p>You will be charged 100 coins per minute</p>
            </div>
          } */}
      ...
      <Modal className="coin-gained-modal" show={showWarning} backdrop="static" keyboard={false}>
        ...
        <span>It's recommended to not refresh the page when the audio call is going on otherwise it may break the functionalities. If you want to end the audio please click the "End Audio" button.</span>
        <br />
